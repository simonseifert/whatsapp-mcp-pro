...
- `WA_STORE_PATH`: Override the SQLite store dir (Python MCP). Default: `/app/store` (Docker) → `<repo>/whatsapp-bridge/store` (local) → `<repo>/store` (legacy/symlink).
- `WA_SKIP_DB_CHECK`: Set to `1` to skip the import-time existence check on `messages.db`. Required for tests (handled in `tests/conftest.py`).
