...
def recall(
    ...
    Note:
        First call after MCP server boot triggers a one-time embedding-model
        download (~470 MB to ~/.cache/huggingface) and starts indexing the
        message corpus in a daemon thread. With ~7000 messages indexing takes
        roughly 1-2 minutes; results during that window are partial.
    """
    return _recall(
        query=query,
        chat_jid=chat_jid,
