...
func ExtractTextContent(msg *waE2E.Message) string {
	...
	// Media captions (image / video / document). Voice notes have no caption.
...
// directPath is the WhatsApp CDN path used as a download fallback when URL alone
// is not enough (e.g. expired URL, web.whatsapp.net redirects).
func ExtractMediaInfo(msg *waE2E.Message) (mediaType, filename, url, directPath string, mediaKey, fileSHA256, fileEncSHA256 []byte, fileLength uint64) {
	if msg == nil {
