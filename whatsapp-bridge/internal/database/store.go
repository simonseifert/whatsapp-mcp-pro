...
func runMigrations(db *sql.DB) error {
	...
	// Add direct_path column for media re-download fallback when URL alone fails.
	_, err = db.Exec(`ALTER TABLE messages ADD COLUMN direct_path TEXT`)
	if err != nil && err.Error() != "duplicate column name: direct_path" {
		fmt.Printf("Warning: migration error (direct_path column): %v\n", err)
	}
	return nil
}
