...
// handleDownloadMedia handles POST /api/download for retrieving message media.
//
// Request body:
//   - message_id: WhatsApp message ID (required)
//   - chat_jid:   chat JID the message belongs to (required)
//
// Response:
//   - success:    boolean
//   - path:       absolute path to the decrypted file on disk (on success)
...
func (s *Server) handleDownloadMedia(w http.ResponseWriter, r *http.Request) {
	...
	path, mediaType, err := s.client.DownloadMessageMedia(r.Context(), s.messageStore, mediaDir, req.MessageID, req.ChatJID)
	w.Header().Set("Content-Type", "application/json")
	if err != nil {
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"success": false,
