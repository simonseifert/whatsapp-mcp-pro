...
- [x] **Presence policy decision** (2026-07-19) — resolved to **off by default**. Both the on-connect `SetPresence("available")` and the periodic ping are now gated behind `PRESENCE_PING_ENABLED` (default `false`), so the paired phone keeps its push notifications and the bridge stops painting an always-online fingerprint. Opt back in with `PRESENCE_PING_ENABLED=true` if you want to appear online to contacts. Future option (not done): mirror real activity mautrix-style.
